...
from simulation_results import Simulation
...
def plot_time_to_reliability(path_to_load,eta_array,window_array,t_sim,**kwargs):
            ...
            for eta in eta_array[p]:
                ...
                for window in window_array[p]:
                    with open(fr"{path}/simulation_{float(eta)}_window_size_{window}_{r}.pkl", "rb") as input_file:
                        ...
                        theta1 = 5e-1
                        constraint = e.alpha
                        #print(np.max(1-e.alpha_min))
                        if enable_bound_plot==True:
                            gamma = e.gamma
                            theta1 = e.theta1

                        #alpha = np.max(e.alpha_max)
                        #if (alpha!=None):
                        #    if alpha<alpha_min:
                        #        alpha_min = alpha
                        ...
                        if frame_flag==True:
                            ...
                            print(np.mean(e.reliability_per_frame))
                        else:
                            reliability_array = np.mean(e.average_reliability,axis=0)

                        #reliability_array =zero_repeated_runs_np(reliability_array)
                        ...
                        axis = np.linspace(start=0,stop=F,num=F)
    ...
    if plot_per_rel==False:
       # plt.plot(np.repeat(lower_bound + constraint, window), linestyle='dashed')  # ,color=color)
        plt.plot(np.repeat(upper_bound + constraint, window), linestyle='dashed')  # ,color=color)
    ...
    #0.13084500000000002
    xx_rel = avg_reliability[0,300:]

    rk = 0.13
    max_viol = rk+0.05*rk
    print(f'Outage probability: {float(np.sum(reliability_array[:]>(max_viol))/len(reliability_array[:]))}')
    print(f'Average violation: {float(np.sum(np.maximum(0,reliability_array[:]-rk)))/np.sum(reliability_array[:]>rk)}')
...
def plot_energy_precision_trade_off(path_to_load,eta_array,window_size,t_sim,trans_end=0,**kwargs):
    ...
    trans_end_array = trans_end#*np.ones((eta_array.shape[1],len(window_array))).astype(np.int32)

    #if window_array.ndim==1:
    #    trans_end_array = trans_end_array.reshape(1,-1)
            ...
            for eta in eta_array[p]:
                ...
                for r in range(start_rel,start_rel+realization_number):
                    with open(fr"{path}/simulation_{float(eta)}_window_size_{int(window)}_{r}.pkl", "rb") as input_file:
                        ...
                        #precision_tracker = np.mean(precision_tracker,axis=1)



                        precision_tracker = precision_tracker[precision_tracker>=0]

                        print(np.mean(precision_tracker))

                        #precision_tracker = precision_tracker[::window]
...
if __name__=="__main__":
    ...
    matlab_out = './matlab'
    #path = ['LOComparisons/alpha_est_2']#,'LOComparisons/CLOMultiUser/MinimalTopology/FNR013']

    #path = ['LOComparisons/alpha_est_2']#,'LOComparisons/CLOMultiUser/MinimalTopology/FNR013']

    path = ['./WiOPTSimCLO15k/','./WiOPTSimLyapunov15k/']

    if choice==1:
        plot_time_to_reliability(path, eta_array, window_array, t_sim,matlab_out=matlab_out,trans_end=0,n_rel=1,B=1,b=1,m=(0),M=(1+0.5),plot_per_rel=True,output_dir='./matlab_plotting',frame_flag=True)
    elif choice==2:
